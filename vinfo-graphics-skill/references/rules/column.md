...
**label position 可选值**：`inside` | `top` | `bottom`
...
| `bottom`      | `inside`       | icon 在柱底，label 在柱体内部                      |
| `top`         | `inside`       | icon 在柱顶，label 在柱体内部                      |
...
| ------------------------ | ------------------------------------ | ------------ | --------------------- | ------------------------------ |
| 人物数据（净资产、成就） | 从预置图片库选择人物相关图片         | `true`       | `center` / `middle`   | 人物叠加在柱子前，视觉冲击力强 |
| 品牌/产品数据            | 从预置图片库选择品牌 logo 或产品图   | `false`      | `right` / `bottom`    | 低调装饰不遮挡数据             |
| 抽象/通用数据            | 从预置图片库选择主题相关图片         | `false`      | `right` / `bottom`    | 轻装饰提升美感                 |

### 注意事项

- `asForeground: true` 时，brandImage 会遮挡部分柱子，确保 label 仍可读
- 前景模式下 brandImage 宽高建议不超过画布的 50%，避免完全遮挡数据
- 从预置图片库 `references/images/images.json` 的 `illustrations` 分类中选择与数据主题匹配的图片
...
| 参数          | 推荐值           | 说明                   |
| ------------- | ---------------- | ---------------------- |
| `from`        | `"currentColor"` | 使用当前颜色作为起始色 |
| `to`          | `"currentColor"` | 使用当前颜色作为结束色 |
| `fromOpacity` | `0.8` ~ `1.0`    | 底部不透明度（深色端） |
| `toOpacity`   | `0.3` ~ `0.5`    | 顶部不透明度（浅色端） |
...
从 `references/images/images.json` 的 `backgrounds` 分类中选择：

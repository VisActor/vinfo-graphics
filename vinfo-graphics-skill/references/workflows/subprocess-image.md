...
> **核心原则**：匹配则使用，不匹配则跳过（不降级搜索）。

---

## 原则

1. **精确匹配**：主题与预置分类匹配时才使用图片
2. **不匹配则跳过**：无匹配分类时不生成背景图/插图，不降级
3. **去重**：background 和 brandImage 使用不同图片，数据项之间图片也应不同
...
1. **不生成 background.image** — 跳过，不要使用其他分类的图片
2. **不生成 brandImage** — 跳过，保持 `visible: false` 或不配置

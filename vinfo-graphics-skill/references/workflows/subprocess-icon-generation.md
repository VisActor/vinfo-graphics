...
| **品牌/平台**     | 微信, 抖音, GitHub, Twitter                | `simple-icons`（3412, 单色可调色）→ `fa7-brands` → `logos`（彩色）         | 每个品牌名作为各自的搜索词                                 |
...
| **品牌/公司**     | `simple-icons`（3412, CC0, 单色品牌标志） | `fa7-brands`（586）, `logos`（1837, 彩色）                                                 | `simple-icons` 覆盖最广且单色可调色；`logos` 为彩色不可调色         |
